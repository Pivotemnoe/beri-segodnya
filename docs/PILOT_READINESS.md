...
- Basic Auth поверх публичного staging либо отдельно утверждённый публичный prelaunch-контур с `noindex`.

## Блокирует публичный production

- не завершена независимая проверка опубликованных документов, РКН, локализации персональных данных и договора с хостингом;
- live уже доступен без Basic Auth и принимает формы; эта граница ещё не утверждена как допустимая;
...
1. Независимо юридически проверить опубликованные реквизиты оператора, consent/policy/terms, решение по РКН/локализации и договор с хостингом.
2. Либо восстановить Basic Auth до завершения внешних ворот, либо письменно принять текущий публичный prelaunch-контур с открытыми формами.
